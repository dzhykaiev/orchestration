...
export default function NewCompanyPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      const { workspace: company } = await api.companies.create({
        name: trimmedName,
      ...
      try {
        ...
        const { agent } = await api.companies.createAgent(company.id, {
        ...
        const { ticket } = await api.companies.createTicket(company.id, {
        ...
        await api.companies.delete(company.id).catch(() => undefined);
        throw agentError;
      }
  ...
  return (
    <div>
      ...
      <div className="company-create-card" style={{ maxWidth: 700 }}>
        <form onSubmit={handleCreate} className="flex flex-col gap-4">
          ...
          <div>
            <label className="label" htmlFor="company-goal">
              Company Goal
            </label>
            <textarea
              id="company-goal"
