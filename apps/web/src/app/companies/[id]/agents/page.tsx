...
export default function CompanyAgentsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [companyRes, agRes, projectsRes, featuresRes] = await Promise.all([
      ...
      setAgents([]);
      setUsingInferredAgents(false);
      setLoadError(getErrorMessage(error, "Failed to load company agents"));
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      const { agent } = await api.companies.createAgent(id, {
        name: form.name.trim(),
        role: form.role,
        tier: form.tier,
  ...
  if (loading) {
    return <PageLoadingState title="Agent Hierarchy" subtitle="Loading company agents..." />;
  }

  if (loadError || !company) {
    return (
      <PageErrorState
        title="Agent page failed to load"
        message={loadError ?? "Company not found"}
    ...
    <div className="workspace-shell">
      <div className="workspace-overview card">
        <div className="workspace-overview-copy">
          <NextActionPanel
            ...
            description="Define who plans, who leads execution, and which specialists the company can delegate to. Explicit agents are useful when you need stable prompts and reliable routing."
            titleTag="h1"
            eyebrowClassName="workspace-overview-eyebrow"
          ...
          <div className="workspace-meta-row">
            ...
            <span className="workspace-meta-pill">{featureCount} company tickets</span>
          </div>
        ...
        <div className="card workspace-context-card">
          ...
          <ul className="workspace-context-list">
            ...
            <li>
              ...
              <strong>When you want to inspect how the company has behaved so far</strong>
            </li>
            <li>
      ...
      {usingInferredAgents && (
        <div className="workspace-info-banner">
          <p>
            Showing inferred hierarchy from existing task history. Create company agent
      ...
      {agents.length === 0 ? (
        <div className="agents-empty">
          ...
          <p className="agents-empty-desc">
            Define explicit agents if this company needs stable delegation rules, or run a project
